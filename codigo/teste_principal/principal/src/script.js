...
async function loadProgressData() {
  const response = await fetch('dbprogress.json');
  const data = await response.json();
  return data;
}

function initializeProgress(data) {
  data.Progress.forEach((chapter, index) => {
      const chapterId = `P${index + 1}`;
      const progressBar = document.getElementById(chapterId);
      progressBar.style.width = chapter[`barra${index + 1}`] + '%';
  });
}

function updateProgress(chapterIndex, data) {
  const chapter = data.Progress[chapterIndex];
  const progressBarId = `P${chapterIndex + 1}`;
  const progressBar = document.getElementById(progressBarId);

  let currentWidth = chapter[`barra${chapterIndex + 1}`];
  if (currentWidth < 100) {
      currentWidth += 10;
      chapter[`barra${chapterIndex + 1}`] = currentWidth;
      progressBar.style.width = currentWidth + '%';
  }
}

document.getElementById('passFcap1').addEventListener('click', async () => {
  const data = await loadProgressData();
  updateProgress(0, data);
});

document.getElementById('passFcap2').addEventListener('click', async () => {
  const data = await loadProgressData();
  updateProgress(1, data);
});

document.getElementById('passFcap3').addEventListener('click', async () => {
  const data = await loadProgressData();
  updateProgress(2, data);
});

document.getElementById('passFcap4').addEventListener('click', async () => {
  const data = await loadProgressData();
  updateProgress(3, data);
});
